...
def check_socket_path(context, *args, **kwargs):
    ...
    if not os.path.exists(socket_path):
        raise RuntimeError(
            ...
            f"Please ensure dvrk_console stereo_display is running and configured with a unixfdsink for stream '{stream}'."
        )
...
def generate_launch_description():
    viewer_name_arg = DeclareLaunchArgument(
        ...
        description="Viewer name used by stereo_display for implicit unixfd socket naming",
    )
